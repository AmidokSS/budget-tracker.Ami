...
## ☁️ Развёртывание на Vercel (рекомендуется)

1. Авторизуйтесь на https://vercel.com и создайте новый проект, привяжите репозиторий GitHub (или загрузите локальную папку).
2. В настройках проекта установите переменные окружения (если используются) — например, DATABASE_URL.
3. Убедитесь, что в настройках сборки выбраны:
	- Framework: Next.js
	- Build command: npm run build
	- Output directory: .next
4. Нажмите Deploy — сайт будет доступен по HTTPS, PWA-манифест и service worker начнут работать корректно.

Локально вы также можете проверить production сборку перед деплоем:
...
Примечание: для корректной работы PWA и появления системного баннера установки необходимо тестировать на реальном устройстве (Chrome на Android или Edge), либо на HTTPS-версии приложения (Vercel предоставляет HTTPS по умолчанию).
